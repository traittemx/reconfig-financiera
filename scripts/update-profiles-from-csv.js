...
async function main() {
    try {
        console.log('Fetching Organizations from Appwrite for ID mapping...');
        const res = await db.listDocuments('finaria', 'organizations');
        const appwriteOrgMap = {}; // AppwriteID -> Slug
        res.documents.forEach(doc => {
            appwriteOrgMap[doc.$id] = doc.slug;
        });

        console.log('Reading organizations.json for UUID mapping...');
        const insforgeOrgs = JSON.parse(fs.readFileSync('organizations.json', 'utf8'));
        const slugToUuidMap = {}; // Slug -> UUID
        insforgeOrgs.forEach(org => {
            slugToUuidMap[org.slug] = org.id;
        });

        // AppwriteID -> UUID
        const orgIdMap = {};
        for (const [awId, slug] of Object.entries(appwriteOrgMap)) {
            if (slugToUuidMap[slug]) {
                orgIdMap[awId] = slugToUuidMap[slug];
            }
        }
        console.log('ID Mapping:', orgIdMap);

        console.log('Reading profiles.json...');
        let profiles = JSON.parse(fs.readFileSync('profiles.json', 'utf8'));

        // 1. Build initial Map from existing specific "Migrated User" profiles
        const userMap = {}; // AppwriteID -> UUID
        profiles.forEach(p => {
            if (p.full_name && p.full_name.startsWith('Migrated User ')) {
                const awId = p.full_name.replace('Migrated User ', '');
                if (awId.length === 20) {
                    userMap[awId] = p.id;
                }
            }
        ...
        lines.forEach(line => {
            ...
            const appwriteId = parts[0];
            let fullName = parts[4];
            const orgId = parts[5];
            const role = parts[6];
            const startDate = parts[7] !== 'null' ? parts[7] : null;
            ...
            // Resolve UUID
            let uuid = userMap[appwriteId];
            let profile = profiles.find(p => p.id === uuid);
            ...
            if (profile) {
                ...
                if (orgId && orgId !== 'null' && orgIdMap[orgId]) {
                    profile.org_id = orgIdMap[orgId];
                }
                updatedCount++;
            } else {
                // Create
                console.log(`Creating new profile for ${appwriteId}...`);
                uuid = crypto.randomUUID();
                userMap[appwriteId] = uuid; // Add to map

                const newProfile = {
                    id: uuid,
                    full_name: fullName,
                    org_id: (orgId && orgId !== 'null' && orgIdMap[orgId]) ? orgIdMap[orgId] : null,
                    role: role,
                    start_date: startDate,
                    updated_at: parts[3], // $updatedAt
                    avatar_url: null,
                    // Store Appwrite ID in a way we can track? 
                    // We rely on "Migrated User" naming convention for mapping, but here we have real names.
                    // We need a separate map for org_members fix.
                };
                profiles.push(newProfile);
                // updating userMap effectively
            }
        });

        // Re-build User Map (Appwrite ID -> UUID)
        // We need to know Appwrite ID. 
        // For existing profiles, it's in "Migrated User ..." (wait, we overwrote the name!)
        // CRITICAL: We overwrote the name in the loop above. We lost the connection!
        // FIX: We must build the map BEFORE overwriting names.
        // OR: We use the `appwriteId` from the CSV loop to populate a map `awId -> uuid`.

        console.log(`Saving ${profiles.length} profiles...`);
        fs.writeFileSync('profiles.json', JSON.stringify(profiles, null, 2));
        console.log(`Updated ${updatedCount} profiles, Created ${createdCount} new profiles.`);
