...
## InsForge Backend Tasks (MCP = CLI for backend)

This project uses InsForge as the backend. For **backend and infrastructure** work, use the **InsForge MCP server** (`user-insforge`) via `call_mcp_tool` — that is the primary way to run SQL, manage schema, deploy functions, manage storage buckets, and get backend metadata. The project skill **insforge-backend** (`.cursor/skills/insforge-backend/`) describes which MCP tools to use for each task. Always read the tool schema in `mcps/user-insforge/tools/<name>.json` before calling. Before writing or changing InsForge SDK code, call **fetch-docs** or **fetch-sdk-docs** to get current API docs.
